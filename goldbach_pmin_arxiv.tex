...
Notably, Oliveira e Silva et al.\ achieved this verification using a \textit{minimal partition strategy}: for each even $N$, they searched for the smallest prime $p$ such that $N - p$ is also prime. This approach is far more efficient than searching from $N/2$ downward, but no theoretical analysis has explained \textit{why} it works so well.

In this paper, we investigate precisely this question: \textbf{How small can the smaller prime be?}
