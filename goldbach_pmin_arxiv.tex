...
\caption{Predicted maximum $p_{\min}$ at various scales, extrapolated using the formula $0.2 \times \ln(N)^3$. Even at the astronomical scale of $10^{100}$ (a googol), the predicted max $p_{\min}$ is only about 2.4 million---requiring a computation that takes microseconds on modern hardware.}
\label{fig:scaling}
\end{figure}
